...
export async function orgScopeFromParam(req: Request, res: Response, next: NextFunction) {
  ...
  if (!caller?.userId) {
    res.status(401).json(makeError("UNAUTHENTICATED", "Missing caller")); return;
  }

  const rows = await db.select().from(memberships).where(eq(memberships.userId, caller.userId));

  if (rows.length === 0) {
    res.status(403).json(makeError("FORBIDDEN", "Not a member of this organization")); return;
  }

  const orgId = rows[0].orgId as string | undefined;

  if (!orgId) {
    res.status(403).json(makeError("FORBIDDEN", "Unable to resolve organization for user")); return;
  }
...
export function requireOrgParamMatch(req: Request, res: Response, next: NextFunction) {
  ...
  if (!caller?.orgId) {
    res.status(403).json(makeError("FORBIDDEN", "Organization not resolved for caller")); return;
  }
  
  if (paramOrgId !== caller.orgId) {
    res.status(403).json(makeError("FORBIDDEN", "Cross-org access denied")); return;
  }
  next();
}
